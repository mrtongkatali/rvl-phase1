...
$pdf->AddPage( 'P', 'LETTER');
	...
	/*
		...
		$tax = $basic_pay * .1;
		$philhealth = $basic_pay * .01;
		$sss = $basic_pay * .05;
		
		$deduction = $tax + $philhealth + $sss;
		$profit = $basic_pay + $allowance + $bonuses;

		$netpay = $basic_pay - ($tax + $philhealth + $sss);

	*/

		$basic_pay 	= number_format($gross_pay,2,'.',',');
		$allowance 	= number_format(0,2,'.',',');
		$bonuses 	= number_format(0,2,'.',',');
		
		$tax 		= number_format($witholding,2,'.',',');
		$philhealth = number_format($philhealth,2,'.',',');
		$sss 		= number_format($pagibig,2,'.',',');
		
		$deduction 	= number_format($total_deduction,2,'.',',');
		$profit 	= number_format($gross_pay,2,'.',',');
