...
foreach($billing_data as $key=>$value):
	...
	#debug_array($value);
		...
		$pdf->Cell(0, 0, 'CAR CARRIER SERVICES FOR PERIOD:  FEBRUARY 23-29, 2008', 0, 1, 'C', 0, '', 0, 0, 'T', 'M');
		...
		$tw = array(20,	#0
			...
			$pdf->Cell($tw[0], 0, "DR No.", 'LRTB', 0, 'C');
		...
		foreach($value['vn_list'] as $a=>$b):
			$pdf->Cell($tw[0], 0, str_pad($b['dr_no'],10,"0",STR_PAD_LEFT), 'LRTB', 0, 'C');
		...
		$pdf->Cell(array_sum($tw)-($tw[9]+$tw[8]+$tw[7]+$tw[6]+$tw[5]+$tw[4]), 0, "", 0, 0, 'R');
		$pdf->Cell($tw[5] + $tw[4], 0, "", 'R', 0, 'C');
